...
namespace Execution
{
	...
	static auto get_extraspace(lua_State* ls) -> ExtraSpace_t*
	{
		//    LOG_FUNC_CALL();
	...
	static auto execute_script(__int64 state, std::string source) -> void
	{
		...
		/*	const auto shared_context = get_extraspace(execution_thread)->shared->script_context;
			...
			auto ltr = new live_thread_ref;
			{
				ltr->th = execution_thread;
			}



			Defs::scriptcontext_resume(shared_context + 1288, out , &ltr, 0, 0, nullptr);*/
		}
	}
	...
	static auto initialize_api(lua_State* script_state) -> void
	{
		...
		lua_pushboolean(script_state, true);
		lua_setfield(script_state, LUA_GLOBALSINDEX, "IS_SERIALITY");

		lua_pushboolean(script_state, true);
		lua_setfield(script_state, LUA_GLOBALSINDEX, "IsSeriality");

		lua_pushboolean(script_state, true);
		lua_setfield(script_state, LUA_GLOBALSINDEX, "SERIALITY_LOADED");

		lua_pushboolean(script_state, true);
		lua_setfield(script_state, LUA_GLOBALSINDEX, "SerialityLoaded");

		lua_pushboolean(script_state, true);
		lua_setfield(script_state, LUA_GLOBALSINDEX, "Seriality");
		...
		Enviroment::push_global(script_state, xorstr_("is_seriality_closure"), Enviroment::Functions::Closures::is_executor_closure);
		Enviroment::push_global(script_state, xorstr_("is_serialityclosure"), Enviroment::Functions::Closures::is_executor_closure);
		Enviroment::push_global(script_state, xorstr_("isseriality_closure"), Enviroment::Functions::Closures::is_executor_closure);
		Enviroment::push_global(script_state, xorstr_("isserialityclosure"), Enviroment::Functions::Closures::is_executor_closure);
	...
	static auto initialize() -> void
	{
		...
		lua_pop((lua_State*)TaskScheduler::get_lua_state(), 1); /* pops the lua state pushed onto the stack */
